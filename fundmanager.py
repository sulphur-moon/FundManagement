...
def show_current_fund(event):
	...
	cursor.execute("SELECT date, amount FROM current WHERE fund='{}'".format(fund_name))
	res = cursor.fetchall()
	# print(res)
	total = 0
	for row in res:
...
def fund_profit(event):
	...
	cursor.execute("SELECT date, buy_amount, sold_amount, profit_amount FROM profit WHERE fund='{}'".format(fund_name))
	res = cursor.fetchall()
	total_buy, total_sold, total_profit = 0, 0, 0
	for row in res:
		date, buy, sold, profit = row
		table_profit.insert('', 'end', values=(date, buy/100, sold/100, profit/100))
		total_buy += buy
		total_sold += sold
		total_profit += profit
	table_profit.insert('', 'end', values=("总金额", total_buy/100, total_sold/100, total_profit/100))
	return
...
def button_buy():
	...
	cursor.execute("INSERT INTO current(date, amount, fund) VALUES('{}', {}, '{}')".format(dt, amount, fund))
	cursor.execute("INSERT INTO history(date, amount, fund, sold) VALUES('{}', {}, '{}', 0)".format(dt, amount, fund))
	connect.commit()
	# 更新持有基金列表
...
def button_sell():
	...
	cursor.execute("SELECT sum(amount) FROM current WHERE fund='{}'".format(fund))
	res = cursor.fetchall()
	buy_amount = res[0][0]
	profit_amount = sold_amount - buy_amount
	print("sold", dt, buy_amount, sold_amount, profit_amount, fund)
	cursor.execute("INSERT INTO profit(date, buy_amount, sold_amount, profit_amount, fund) VALUES('{}', {}, {}, {}, '{}')".format(dt, buy_amount, sold_amount, profit_amount, fund))
	cursor.execute("DELETE FROM current WHERE fund='{}'".format(fund))
	cursor.execute("INSERT INTO history(date, amount, fund, sold) VALUES('{}', {}, '{}', 1)".format(dt, sold_amount, fund))
	connect.commit()
	# 更新基金列表
	fund_set_current = set()
	res = cursor.execute("SELECT DISTINCT fund FROM current")
...
table_current_fund = ttk.Treeview(frame, show="headings", yscrollcommand=scrollBar.set)
...
btn_buy = tkinter.Button(mainwindow, text ="买入", width=10, height=2, command=button_buy)
btn_buy.place(x=580, y=500)
btn_sell = tkinter.Button(mainwindow, text ="卖出", width=10, height=2, command=button_sell)
